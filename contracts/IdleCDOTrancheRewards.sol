...
contract IdleCDOTrancheRewards is
    Initializable,
    PausableUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable,
    IIdleCDOTrancheRewards,
    IdleCDOTrancheRewardsStorage
{
    ...
    function initialize(
        ...
        require(
            _owner != address(0) &&
                _trancheToken != address(0) &&
                _idleCDO != address(0) &&
                _governanceRecoveryFund != address(0),
            "IS_0"
        );
    ...
    function _stake(
        ...
        IERC20Detailed(tranche).safeTransferFrom(
            _payer,
            address(this),
            _amount
        );
        ...
        returns (uint256)
    {
        ...
        return
            ((_globalIdx - _userIdx) * usersStakes[user]) / ONE_TRANCHE_TOKEN;
    }
        ...
        returns (uint256 _index)
    {
        ...
        if (_totalStaked > 0 && _lockedRewards > 0) {
            ...
            if (distance < coolingPeriod) {
                ...
                // still be locked
                uint256 unlockedRewards = (_lockedRewards * distance) /
                    coolingPeriod;
    ...
    function depositReward(address _reward, uint256 _amount) external override {
        ...
        IERC20Detailed(_reward).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
        if (totalStaked > 0) {
            ...
            rewardsIndexes[_reward] +=
                (_amount * ONE_TRANCHE_TOKEN) /
                totalStaked;
        }
        ...
        for (uint256 i = 0; i < _rewards.length; i++) {
            ...
            if (_currStake == 0) {
                ...
                usersIndexes[_user][reward] =
                    ...
                    (_amountToStake * (rewardsIndexes[reward] - userIndex)) /
                        (_currStake + _amountToStake));
            }
        }
    }
    ...
    function transferToken(address token, uint256 value)
        external
        onlyOwner
        nonReentrant
    {
