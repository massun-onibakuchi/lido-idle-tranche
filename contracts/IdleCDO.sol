    ...
    IdleCDOStorage
{
    ...
    function getContractValue() public view override returns (uint256) {
        address _strategyToken = strategyToken;
        uint256 strategyTokenDecimals = IERC20Detailed(_strategyToken)
            .decimals();
        ...
        return
            ((_contractTokenBalance(_strategyToken) * _strategyPrice()) /
                (10**(strategyTokenDecimals))) +
    ...
    function virtualPrice(address _tranche)
        public
        view
        returns (uint256 _virtualPrice)
    {
    ...
    function _updateAccounting() internal {
        ...
        (uint256 _priceAA, uint256 _totalAAGain) = _virtualPricesAux(
            AATranche,
            nav,
            _lastNAV,
            _lastNAVAA,
        ...
        (uint256 _priceBB, uint256 _totalBBGain) = _virtualPricesAux(
            BBTranche,
            nav,
            _lastNAV,
            _lastNAVBB,
    ...
    function _virtualBalance(address _tranche) internal view returns (uint256) {
        ...
        return
            (IdleCDOTranche(_tranche).totalSupply() * virtualPrice(_tranche)) /
            ONE_TRANCHE_TOKEN;
    }
    ...
    function _depositFees() internal returns (uint256 _currAARatio) {
        ...
        if (_amount > 0) {
            ...
            if (isStakingRewardsActive) {
                IIdleCDOTrancheRewards(stakingRewards).stakeFor(
                    _feeReceiver,
                    _minted
                );
            }
    ...
    function _liquidate(uint256 _amount, bool _revertIfNeeded)
        internal
        returns (uint256 _redeemedTokens)
    {
    ...
    function _updateIncentives(uint256 currAARatio) internal {
        ...
        if (
            _isBBStakingActive &&
            (currAARatio > (_trancheIdealWeightRatio + _idealRange))
        ) {
    ...
    function _depositIncentiveToken(address _stakingContract, uint256 _ratio)
        internal
    {
        ...
        for (uint256 i = 0; i < _incentiveTokens.length; i++) {
            ...
            uint256 _reward = (_contractTokenBalance(_incentiveToken) *
                _ratio) / FULL_ALLOC;
            if (_reward > 0) {
    ...
    function _sellReward(
        ...
        IERC20Detailed(_rewardToken).safeIncreaseAllowance(
            address(_uniRouter),
            _amount
        );
    ...
    function _sellAllRewards(
        address _token,
        uint256[] memory _sellAmounts,
        uint256[] memory _minAmount,
        bool[] memory _skipReward
    )
        internal
        returns (
            uint256[] memory _soldAmounts,
        ...
        )
    {
        ...
        address[] memory _rewards = IIdleCDOStrategy(strategy)
            .getRewardTokens();
        address _rewardToken;
        ...
        for (uint256 i = 0; i < _rewards.length; i++) {
            ...
            if (
                _skipReward[i] ||
                _includesAddress(_incentiveTokens, _rewardToken)
            ) {
                continue;
            }
            ...
            (_soldAmounts[i], _swappedAmounts[i]) = _sellReward(
                _rewardToken,
                _path,
                _sellAmounts[i],
                _minAmount[i]
    ...
    function _getApr(address _tranche, uint256 _AATrancheSplitRatio)
        internal
        view
        returns (uint256)
    {
    ...
    function _lockedRewards() internal view returns (uint256 _locked) {
        ...
        if (
            ...
            _locked =
                (_harvestedRewards *
                    (_releaseBlocksPeriod - _blocksSinceLastHarvest)) /
                _releaseBlocksPeriod;
        }
    }
    ...
    function _claimStkAave() internal {
        ...
        if (_stakersCooldown > 0) {
            ...
            if (
                _stakersCooldown + _stkAave.COOLDOWN_SECONDS() < block.timestamp
            ) {
    ...
    function harvest(
        ...
        returns (
            uint256[] memory _soldAmounts,
            uint256[] memory _swappedAmounts,
            uint256[] memory _redeemedRewards
        )
    {
        ...
        if (!_skipRedeem || !_skipIncentivesUpdate || !_skipFeeDeposit) {
            ...
            if (!_skipRedeem) {
                ...
                _claimStkAave();
                // Sell rewards
                (_soldAmounts, _swappedAmounts, _totSold) = _sellAllRewards(
                    _token,
                    _sellAmounts,
                    _minAmount,
                    _skipReward
            ...
            if (!_skipIncentivesUpdate) {
                ...
                _updateIncentives(
                    currAARatio == 0 ? getCurrentAARatio() : currAARatio
                );
            }
        }
    ...
    function liquidate(uint256 _amount, bool _revertIfNeeded)
        external
        returns (uint256)
    {
        require(msg.sender == rebalancer || msg.sender == owner(), "6");
    ...
    function setStrategy(address _strategy, address[] memory _incentiveTokens)
        external
        onlyOwner
    {
    ...
    function setIncentiveTokens(address[] memory _incentiveTokens)
        external
        onlyOwner
    {
        incentiveTokens = _incentiveTokens;
    ...
    function setStakingRewards(address _AAStaking, address _BBStaking)
        external
        onlyOwner
    {
    ...
    function _contractTokenBalance(address _token)
        internal
        view
        returns (uint256)
    {
    ...
    function _allowUnlimitedSpend(address _token, address _spender) internal {
        IERC20Detailed(_token).safeIncreaseAllowance(
            _spender,
            type(uint256).max
        );
    }
    ...
    function _checkSameTx() internal view {
        require(
            keccak256(abi.encodePacked(tx.origin, block.number)) !=
                _lastCallerBlock,
            "8"
        );
    }
