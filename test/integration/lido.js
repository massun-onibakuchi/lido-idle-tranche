...
describe("IdleLidoStrategy", function () {
  beforeEach(async () => {
    ...
    // Params
    const initialAmount = BN("10").mul(ONE_TOKEN(18));
  ...
  it("should deposit", async () => {
    ...
    expect(initialBal.sub(finalBal)).to.equal(_amount);
    expect(finalWstEthBal.sub(initialWstEthBal)).to.be.closeTo(_outputWstEth, 1);
  ...
  it("should redeem", async () => {
    ...
    expect(initialWstEthBal.sub(finalWstEthBal)).to.be.closeTo(_outputWstEth, 1);
    expect(finalBal.sub(initialBal)).to.be.closeTo(_amount, 1);
  ...
  it("should return the current net apr", async () => {
    const [postTotalPooledEther, preTotalPooledEther, timeElapsed] = await oracle.getLastCompletedReportDelta()
    const feeBps = await lido.getFee()
